...
*         : 21.11.2023 1.03     Added compile switch of BSP_CFG_BOOTLOADER_PROJECT.
...
*         : 27.11.2024 1.04     The following changes have been made to apply the Technical Update Information
*                               (TN-RX*-A0281A):
*                               - The RCR4.RCKSEL bit setting process has been modified to only occur when the RTC is 
*                                 not in use.
*                               - Deleted the setting process of the SOSCCR2.SOSTP2 bit.
...
uint32_t get_iclk_freq_hz(void)
{
    ...
    switch (cksel)
    {
        ...
        case BSP_PRV_CKSEL_PLL:
            ...
            /* The RX671 have two possible sources for the PLL */
...
/* Disable the following functions in the bootloader project. */
    ...
    if (1 == SYSTEM.BKPSR.BIT.PDDF)
    {
        /* If the Battery backup power voltage drop detected, Initialize of Sub-Clock. */
#endif

#if BSP_CFG_BACKUP_DOMAIN_SUB_CLOCK_SOURCE == 0 /* Internal sub-clock oscillator */
        SYSTEM.BKSCCR.BYTE = 0x00;
#elif BSP_CFG_BACKUP_DOMAIN_SUB_CLOCK_SOURCE == 1 /* External sub-clock oscillator */
        SYSTEM.BKSCCR.BYTE = 0x40;
#else
    #error "Error! Invalid setting for BSP_CFG_BACKUP_DOMAIN_SUB_CLOCK_SOURCE in r_bsp_config.h"
#endif
...
#if BSP_CFG_RTC_ENABLE == 0 /* TN-RX*-A0281A */
    #if BSP_CFG_MAIN_CLOCK_OSCILLATE_ENABLE == 1
    ...
    {
        /* If the Battery backup power voltage drop not detected and Sub-Clock used, does not initialize Sub-Clock. */
#if ((BSP_CFG_SUB_CLOCK_OSCILLATE_ENABLE == 0) && (BSP_CFG_RTC_ENABLE == 0))
        /* SOSCCR - Sub-Clock Oscillator Control Register
        b7:b1    Reserved - The write value should be 0.
        b0       SOSTP    - Sub-clock oscillator Stop - Sub-clock oscillator is stopped. */
        SYSTEM.SOSCCR.BYTE = 0x01;
...
#endif /* ((BSP_CFG_SUB_CLOCK_OSCILLATE_ENABLE == 0) && (BSP_CFG_RTC_ENABLE == 0)) */
    ...
    if (1 == SYSTEM.HOCOCR.BIT.HCSTP)
    {
        ...
        if(0x00 == SYSTEM.HOCOCR2.BYTE)      // 16MHz
        {
            SYSTEM.HOCOTRR0.LONG = (0xFFFFFE00 | BSP_CFG_HOCO_TRIMMING_REG_VALUE);
        }
        else if(0x01 == SYSTEM.HOCOCR2.BYTE) // 18MHz
        {
            SYSTEM.HOCOTRR1.LONG = (0xFFFFFE00 | BSP_CFG_HOCO_TRIMMING_REG_VALUE);
        }
        else                                 // 20MHz
        {
            SYSTEM.HOCOTRR2.LONG = (0xFFFFFE00 | BSP_CFG_HOCO_TRIMMING_REG_VALUE);
        }
        #endif

        #if BSP_CFG_HOCO_FLL_ENABLE == 1
        /* Set the multiplication ratio of the FLL reference clock. */
           #if   (BSP_CFG_HOCO_FREQUENCY == 0)
        SYSTEM.FLLCR2.WORD = 0x01E9;
           #elif (BSP_CFG_HOCO_FREQUENCY == 1)
        SYSTEM.FLLCR2.WORD = 0x0226;
           #elif (BSP_CFG_HOCO_FREQUENCY == 2)
        SYSTEM.FLLCR2.WORD = 0x0263;
           #endif

        /* Enable FLL */
        SYSTEM.FLLCR1.BIT.FLLEN = 1;
        #endif
        ...
        #if BSP_CFG_HOCO_FLL_ENABLE == 1
        /* Wait until the FLL operation becomes stable.(maximum is 1.8ms.) */
        R_BSP_SoftwareDelay((uint32_t)1800, BSP_DELAY_MICROSECS);
        #endif
    }
    ...
    /* RX671 has a ROMWT register which controls the cycle waiting for access to code flash memory.
       It is set as zero coming out of reset.
       When setting ICLK to [60 MHz < ICLK <= 120 MHz], set the ROMWT.ROMWT[1:0] bits to 01b. */
    if (BSP_ICLK_HZ > BSP_MCU_ROMWT_FREQ_THRESHOLD_01)
    {
